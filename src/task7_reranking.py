...
def rerank_cross_encoder(
    ...
    if api_key:
        try:
            ...
            print(f"  [WARNING] Jina reranker unavailable: {error}")

    # Deterministic local fallback when no reranker API key is configured.
    ...
    for candidate in candidates:
        ...
        item = candidate.copy()
        item["score"] = 0.7 * overlap + 0.3 * float(candidate.get("score", 0.0))
    ...
    for _ in range(min(top_k, len(candidates))):
        ...
        for index in remaining:
            ...
            diversity_penalty = max(
                (
                    _cosine_similarity(embedding, candidates[selected_idx]["embedding"])
            ...
            if score > best_score:
                best_idx, best_score = index, score
